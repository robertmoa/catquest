...
function buyShopItem(cost, itemName, attackBonus) {
    ...
    if (attackBonus > 0 && window.increaseMaxAttack) {
        window.increaseMaxAttack(attackBonus);
    }
...
function initializeShopButtons() {
    ...
    buyButtons.forEach((button) => {
        button.addEventListener("click", () => {
            ...
            const attackBonus = Number(button.dataset.attackBonus) || 0;
            buyShopItem(cost, itemName, attackBonus);
        });
    });
}
